...
class Query
{
    ...
    protected function appendColumnTableName(string &$s, QueryArgument $d): void
    {
        if ($d->isString()) {
            ...
            $t = $d->getTwoTuple();
            $this->appendColumnTableName($s, $t->getFirstArgument());
            $s .= '.';
            $this->appendColumnTableName($s, $t->getSecondArgument());
            ...
            $this->appendColumnTableName($s, $t->getSecondArgument());
            $s .= ' AS ';
            $this->appendColumnTableName($s, $t->getThirdArgument());
        } else {
            $s .= $d->asString();
        }
    }
    ...
    protected function appendValueClauses(
        ...
        foreach ($param->getPairs() as $pair) {
            ...
            $first_param = false;
            $this->appendColumnTableName($ret, $pair->getFirstArgument());
